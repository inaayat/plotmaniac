...
`renderCountryHistory` in `app.js` (drawer and relation page):

- Chart appears only when some beat has numeric `tone`.
- Each beat may render `links` as chips (`target="_blank"`, `rel="noreferrer"`).
