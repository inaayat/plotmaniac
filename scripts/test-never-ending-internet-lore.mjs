...
assert.ok(avengersBefore.includes("incredible-hulk"), "Incredible Hulk stays in Watch Before, not clipped by a top-N cap");
{
  ...
  assert.ok(fitted.height <= 168 && fitted.height >= 44, `rail posters shrink to fit, got ${fitted.height}`);
...
assert.doesNotMatch(chronoViewSource, /Full chronological list/);
assert.doesNotMatch(chronoViewSource, /onOpenChronology/);
assert.match(chronoViewSource, /chrono-arrow-shaft/);
...
assert.match(css, /grid-template-rows:\s*auto var\(--chrono-reach\) var\(--chrono-hero-h\) var\(--chrono-reach\) auto/, "poster sits between two equal fixed rows so the rails center on it and it never moves");
assert.match(css, /align-self:\s*safe center/, "long side stacks grow downward instead of into the headers");
assert.match(css, /grid-template-rows:\s*subgrid/, "rail and center columns share the layout rows, so headers line up");
assert.doesNotMatch(css, /calc\(50% - 165px\)/, "no poster offsets hard-coded to one poster height");
assert.match(html, /id="view-reset"/);
assert.match(appSource, /function resetChronologyView/);
assert.match(appSource, /setLaneZoom\(1\);\s*render\(\{ push: true \}\);/, "reset returns the chronology zoom to 100%");
...
assert.match(appSource, /button\.dataset\.view === "chronology"/);
