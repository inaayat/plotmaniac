...
A dark field guide. Open the site and pick a person or a country, then read the web of friends and foes or the timeline under it. On a person’s web, people with more timeline beats sit closer to the center, and the ring still refits when the window changes. H3 is the person plot. The United States plot is a current snapshot of foreign relations: major allies wear a green outline and major foes a red one. Neutrals have no colored outline. The other countries stay inside collapsed regions until that region is opened. Choosing a country opens its relationship timeline.
...
5. A country plot can instead set `disclosure` to `regions` and add `paths.countries`. Each country needs `status` (`friend`, `foe`, or `neutral`), `outline` (`green`, `red`, or `none`), `region`, `first_load`, and a `timeline` of `{ year, event }`. The board shows only `first_load` friends and foes. Regions stay collapsed until opened, and a neutral never gets a green or red outline.
