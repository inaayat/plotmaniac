...
2. Register it in `data/plots.json` with a `centerId`, `friendKinds`, and `enemyKinds`. A person plot can add `hubs` (each with `id`, `label`, `centerId`) and tag events with `hubs: ["h3"]` so one page can switch timeline focuses without duplicating people. Set `arrangement` to `hubs` to keep every hub on one web: shared people in the center, each hub just outside, and single-hub people farther out. Set `includeOrbit` when a single-center web should keep people who are not friends or foes of the active center. With more than one plot, the site opens on a picker. Open `?plot=<id>` to jump straight in. `?plot=h3` still opens YouTubers.
...
Friend and enemy color comes from each person’s relationship to the focused hub. A feud or lawsuit outweighs an older collaboration. On the YouTubers web, position follows hub membership instead: the shared middle, then the hubs, then each hub’s own people. Other plots still leave everyone else in “Around the show” when orbit is off the first view. Dated relations are ignored until a year is chosen.
