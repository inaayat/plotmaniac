...
7. A history plot can set `arrangement` to `historical-map` and `paths.reference` to a research file (see `data/partition-of-india/reference.json`). The page is full width: a map, a timeline scrubber, and an overlay inspector. Border changes animate between documented states. Kashmir shows administered territory unless the claims overlay is on. Optional `paths.portraits` adds a Wikimedia portrait only when the file is freely licensed, with `src`, `page`, `author`, `license`, and `licenseUrl`. People without one get a monogram.
8. The wars plot sets `arrangement` to `wars`, a `year` range, `paths.conflicts`, and `paths.world`. The board is a world map fitted to the window. For the chosen year it draws one arc for each pair of states on opposing sides, and lists each war with a link to its Wikipedia article. Civil wars mark the country and do not draw an outbound arc. `scripts/build-wars.py` rebuilds `data/wars/conflicts.json` from the English Wikipedia lists of wars.
